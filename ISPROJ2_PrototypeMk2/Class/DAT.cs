...
namespace PROJ2_PTE.Class
{
    ...
    {
        static string ConnStr = "Server=CHOCOBO\\SQLEXPRESS;Database=ISPROJ2;Trusted_Connection=True;";
        ...
        public static DataTable projectBillingItem() {
            Open();
            string strGet = "Select stock_code AS 'Code', stock_desc AS 'Description',"
                + "stock_location AS 'Location', stock_qty AS 'Quantity',"
                + "stock_price AS 'Cost', stock_bom AS 'Bill of Material', stock_warehouse AS 'Warehouse',"
                + "supplier_name AS 'Supplier', stock_leadtime AS 'Days to be delivered' from Stock INNER JOIN Supplier "
                + "ON Stock.stock_supplier_id = Supplier.supplier_id";
            SqlCommand SCM = new SqlCommand(strGet, SCN);
            SqlDataAdapter SDA = new SqlDataAdapter(SCM);
            DataTable DTB = new DataTable();
            SDA.Fill(DTB);
        ...
        public static DataTable projectBillingTask()
        {
            Open();
            string strGet = "Select proj_code AS 'Code', projTask_descr AS 'Description', "
                + "projTask_startDate AS 'Start Date', projTask_endDate AS 'End Date', "
                + "projTask_actualStart AS 'Actual Start Date', projTask_actualend AS 'Actual End Date', "
                + "projTask_level AS 'Level', projTask_preq AS 'Pre-requisite', status AS 'Status' from ProjectTask";
            SqlCommand SCM = new SqlCommand(strGet, SCN);
            SqlDataAdapter SDA = new SqlDataAdapter(SCM);
            DataTable DTB = new DataTable();
            SDA.Fill(DTB);
